...
import { Card,Result, Image, Tooltip, Modal, Input, Alert, Spin, Button } from "antd";
import { useNFTBalance } from "hooks/useNFTBalance";
import { FileSearchOutlined, ShoppingCartOutlined, DoubleRightOutlined, CloseOutlined } from "@ant-design/icons";
...
const showConfirm = () => {
  confirm({
    title: 'Get Access to the Data Set',
    ...
    onOk() {
      ...
      //Code for Download Data Set with the NFT Hash Function
    },
...
function NFTBalance() {
  ...
  //const AccessFileHash = JSON.parse(uri_link);
  ...
  async function getURI(nft){
    ...
    await contractProcessor.fetch({
      ...
      onSuccess: () => {
        getJSON(message)
        uri_link=data.filehash;
        console.log(uri_link);
        file_link = "https://ipfs.infura.io/ipfs/" + uri_link;
        showConfirm();
      },
      onError: (error) => {
        ...
        {NFTBalance &&
          NFTBalance.map((nft, index) => (
            <Card
              ...
              actions={[
                ...
                <Tooltip title="List NFT for sale">
                 ...
                 <Tooltip title="Access Data">
                 <DoubleRightOutlined onClick={() => getURI(nft)}/>
               </Tooltip>,
              ]}
            ...
            >
              ...
              <Meta title={nft.name} description={nft.name} />
            </Card>
          ))}
      </div>
